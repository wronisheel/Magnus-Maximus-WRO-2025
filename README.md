...
The PCA9685 is an I²C multiplexer that allows a single I²C bus from the Raspberry Pi to be expanded into eight independent downstream channels, each of which can connect to devices with the same I²C address. This is especially useful when working with multiple identical sensors, like the VL53L1X ToF sensors, which all share the same default address. By selecting a channel through simple I²C commands, the Raspberry Pi can communicate with one sensor at a time without address conflicts. Compact and efficient, the PCA9658 enables reliable scaling of sensor arrays, ensures smooth data handling, and provides flexibility to add more I²C devices in future upgrades.
...
For the open round, the robot makes use of VL53L1X Time-of-Flight (ToF) sensors, mounted on the front, left, and right sides of the chassis. These sensors work by emitting laser pulses and calculating the time taken for the reflections to return, providing accurate distance measurements. Since multiple VL53L1X sensors share the same default I²C address, a PCA9548 I²C multiplexer is employed. The multiplexer allows the Raspberry Pi to assign each sensor to a separate I²C channel, ensuring conflict-free communication and smooth operation. This setup not only supports the current three-sensor configuration but also provides flexibility for adding more sensors in the future. The Raspberry Pi continuously monitors these readings to detect objects in real time and decides whether to move forward, steer left, or turn right to avoid collisions. Complementing this, a BNO055 gyro sensor provides orientation feedback, allowing the robot to maintain a stable heading and execute precise turns without drifting. Together, these inputs deliver reliable obstacle awareness and control for navigating open spaces effectively.

The robot also takes on the Open Challenge round, where it must navigate an arena bounded primarily by walls. In this scenario, the VL53L1X sensors again play a crucial role by constantly scanning the distances between the robot and the surrounding walls. With the help of the PCA9548 multiplexer, the Raspberry Pi receives steady, uninterrupted feedback from each sensor, enabling it to calculate the robot’s relative position within the arena. Meanwhile, the BNO055 gyro ensures orientation stability, helping the robot move in straight lines, correct angular deviations, and align itself during sharp turns. This dual reliance on distance sensing and orientation stabilization prevents drifting too close to the walls and ensures smooth, safe navigation in constrained environments.

In the obstacle round, the system shifts its reliance toward vision through the PiCamera3, mounted at the front of the chassis. The camera captures live video of the environment, which is processed on the Raspberry Pi using computer vision libraries such as OpenCV. This enables the robot to recognize objects, detect obstacles, follow paths, or interpret lines drawn on the ground. Unlike the open round where laser sensors dominate, here visual input enables the robot to make more intelligent navigation decisions, adapting dynamically to unpredictable or complex challenges. Even during these vision-based tasks, the BNO055 continues to stabilize motion, ensuring the robot’s orientation remains consistent as it performs avoidance maneuvers or navigates tricky paths. By integrating ToF sensor data via the PCA9548 multiplexer, vision cues from the PiCamera3, and orientation feedback from the BNO055, the robot demonstrates a highly flexible and coordinated navigation approach.
...
All components—sensors, PiCamera3, PCA9548 multiplexer, BNO055 gyro, PCB, battery, and motors—are securely mounted onto the robot’s chassis. The design balances weight and functionality, ensuring stability even when navigating tight spaces or avoiding obstacles. The forward-mounted PiCamera3 provides an unobstructed view of the environment, while the strategically placed VL53L1X sensors maximize coverage of the robot’s immediate surroundings, and the BNO055 contributes to maintaining directional integrity.

By combining the laser-based precision of the VL53L1X sensors managed via the PCA9548 multiplexer for the open and wall-bounded challenge rounds, the orientation stability provided by the BNO055 gyro, and the visual intelligence of the PiCamera3 for the obstacle round, the robot achieves a strong balance of accuracy and adaptability. It not only detects objects directly in its path but also maintains spatial awareness in wall-enclosed environments and intelligently navigates more complex obstacle courses.

This integration of multiplexer-managed sensors, gyro-based orientation, vision, and motor control results in a robot that is both robust and versatile. It can reliably detect and avoid objects, maintain stable navigation between walls, and adapt dynamically to visually complex challenges. Altogether, the system demonstrates smooth, autonomous operation across varied environments, making it highly effective in meeting the demands of all competition rounds.
...
The robot uses the PiCam3 as its core vision system for both wall-following and obstacle navigation. While moving along the course, the camera continuously analyzes wall edges and the orientation of the path in real time, allowing the robot to stay aligned. When the PiCam3 detects the end of a wall, it signals the robot to execute the appropriate turn, and if a wall appears directly ahead, the robot halts, performs the turn, and resumes its course to prevent collisions.

For obstacle detection, the PiCam3 identifies red or green blocks in the robot’s path, while the VL53L1X ToF sensor measures their distance. If the measured distance is greater than 120 cm, the robot advances in the direction indicated by the camera’s detection. Meanwhile, the BNO055 gyro sensor maintains a stable heading, ensuring that the robot travels straight and smoothly after turns or avoidance maneuvers. By combining visual recognition, distance sensing, and orientation control, the robot achieves precise, efficient, and reliable performance in the obstacle round.

## Wiring and Sensor Integration
The wiring and integration of components in the robot are carefully designed to ensure stable power delivery and reliable data communication. The VL53L1X sensors are connected through the PCA9658 I²C multiplexer, which in turn interfaces with the Raspberry Pi 4, preventing address conflicts and allowing multiple distance sensors to operate simultaneously. The BNO055 gyro sensor is also linked via I²C to provide real-time orientation data, ensuring that the robot maintains a stable heading during navigation. The servo motor and EV3 motor, mounted on the EV3 chassis, are powered directly by the 3.7V 2200mAh lithium-ion battery through the custom PCB, which manages connections and distributes power efficiently. The PiCam3, connected to the Raspberry Pi 4, receives its supply through the PCB as well, enabling continuous video capture for vision-based navigation. All wiring paths are routed systematically on the chassis to prevent interference between power and signal lines, ensuring smooth communication between components. This integration of sensors, motors, multiplexer, gyro, and camera through the PCB provides a robust framework that allows the robot to operate reliably and maintain precise control, accurate sensing, and stable performance across all rounds.

While we believe that the task could be accomplished effectively using only two sensors, incorporating a third sensor offers several advantages. Not only does it make the overall process more efficient, but it also simplifies navigation, allowing the robot to react more quickly to environmental changes. By using three VL53L1X sensors routed through the PCA9658, the robot can save valuable time during the task, resulting in smoother and more precise performance. Alongside this, the BNO055 enhances directional stability, ensuring straight runs and accurate turns. These sensors are carefully positioned to detect nearby walls without interfering with mobility, while the gyro adds a stabilizing layer of orientation awareness. This strategic placement and integration strike a balance between optimal functionality and preventing disruptions during movement.

## Obstacle Management

In designing our robot’s obstacle management system, we adopted a multi-sensor strategy that ensures smooth navigation while maintaining efficiency and accuracy. The robot employs the Raspberry Pi Camera 3, VL53L1X laser distance sensors, the BNO055 orientation sensor, and the PCA9658 I²C multiplexer, with each component serving a distinct role depending on the competition round.

During the Open Challenge round, the VL53L1X sensors, routed through the PCA9658 multiplexer, provide precise distance measurement for wall-following and turning in narrow corridors without communication conflicts. At the same time, the BNO055 gyro sensor ensures directional stability by maintaining a consistent heading during straight runs and turns. Meanwhile, in the Obstacle round, the system relies primarily on the PiCamera3 for vision-based detection. The camera identifies red and green objects, estimates their distance, and enables intelligent decisions such as stopping, turning, or changing direction, while the BNO055 continues to stabilize orientation during avoidance maneuvers.

By integrating these specialized approaches, the system demonstrates effective sensor fusion—combining distance sensing through multiplexed VL53L1X units, orientation stabilization with the BNO055, and vision-based object detection with the PiCamera3. This ensures the robot avoids collisions, detects colors accurately, and maintains smooth alignment with the course, making obstacle management both robust and adaptable to different challenges.
